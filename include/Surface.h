...
bool separate_surface_overlapp(Surface& surf1 , Surface& surf2, double edge_movement=-0.25, double smoothing=0.3, int max_iter=400)
{
  ...
  double surf1_ael = 0.4*surf1.average_edge_length(); // surf1.vertices_inside new ??
  double surf2_ael = 0.4*surf2.average_edge_length();
...
bool separate_surface_overlapp(Surface& surf1 , Surface& surf2 , Surface& other, double edge_movement=-0.25, double smoothing=0.3, int max_iter=400)
{
    ...
    double surf1_ael = 0.4*surf1.average_edge_length();
    double surf2_ael = 0.4*surf2.average_edge_length();
...
bool separate_close_surfaces(Surface& surf1 , Surface& surf2, double edge_movement=-0.25, double smoothing=0.1, int max_iter=400)
{
   ...
   double surf1_ael = 0.4*surf1.average_edge_length(); // TODO: test
   double surf2_ael = 0.4*surf2.average_edge_length();
...
class Surface
{
   public:
    ...
    Surface cylindric_extension(const Point_3& p1,double radius, double length, double edge_length ,bool normal=true );      
    Surface cylindric_connection(Surface other, double radius, double edge_length);              
    ...
    std::pair<bool,int> enclose( Surface& other, double adjustment=0.5 , double smoothing=0.1, int max_iter=400); 
    std::pair<bool,int> separate(Surface& other, double adjustment=-0.5, double smoothing=0.3, int max_iter=400); 
    ...
    * @overload 
    */   
   Surface cylindric_extension(double x, double y ,double z, double radius, double length,double edge_length, bool normal)
...
inline Surface::vertex_vector Surface::get_close_vertices(Surface &other)
{
   ...
   for (vertex_descriptor v_it : mesh.vertices())
   {
        ...
        closest = other.get_mesh().point((search.begin()+A)->first); // error FIXME
...
inline std::pair<bool,int> Surface::manipulate_vertex_selection(Surface &other, double adjustment,  double smoothing, int max_iter)
{
   ...
   vertex_vector vertices = this->get_vertices_with_property<A,B>(other) ;
   int after;
   int before = vertices.size();
   
   // lower threshold for edges.
   double ael = 0.4*average_edge_length();
...
inline std::pair<bool,int> Surface::manipulate_vertex_selection(Surface &other, double adjustment,  double smoothing, int max_iter)
{
   ...
   double ael = 0.4*average_edge_length(); // remove edges that are less than 20 % of average edge length.
 ...
 * Separates non-adjacent surface mesh vertices so that the nearest surface mesh vertices are adjacent. 
 * 
 * TODO : Fix issue with close vertices in negaitve normal direction. 
...
inline std::pair<bool,int> Surface::embed(Surface& other, double adjustment,  double smoothing, int max_iter) 
{
   ...
   return manipulate_vertex_selection<CGAL::ON_UNBOUNDED_SIDE,CGAL::ON_UNBOUNDED_SIDE>(other, adjustment,smoothing,max_iter);
...
inline std::pair<bool,int> Surface::enclose(Surface& other, double adjustment,double smoothing, int max_iter) 
{
   ...
   return manipulate_vertex_selection<CGAL::ON_BOUNDED_SIDE,CGAL::ON_BOUNDED_SIDE>(other, adjustment,smoothing,max_iter);
}
 ...
 * number of adjusted vertices. 
 */
inline std::pair<bool,int> Surface::separate(Surface& other, double adjustment, double smoothing, int max_iter)
{ 
    ...
    if (queries.first == queries.second)
      throw  InvalidArgumentError("Surface ust be enclosed or embedded.");
    else
    { 
       if (queries.first) 
          return manipulate_vertex_selection(other, -abs(adjustment),smoothing,max_iter);
       else 
         return manipulate_vertex_selection(other, abs(adjustment),smoothing,max_iter);
    } 
 ...
 */
inline Surface Surface::cylindric_extension(const Point_3& p1, double radius, double length, double edge_length, bool normal)
{
   assert_non_empty_mesh();
   ...
   vertex_vector vertices = get_closest_vertices(p1,1);
   vertices = get_closest_vertices(mesh.point(vertices[0]),8); 
   ...
   cylinder.make_cylinder(x0,y0,z0,x1,y1,z1,radius,edge_length); // NOTE : 
...
inline Surface Surface::cylindric_connection(Surface other, double radius, double edge_length)
{
   assert_non_empty_mesh();
   Surface::vertex_vector results;
   Vertex_point_pmap vppmap = get(CGAL::vertex_point,other.get_mesh());  
  ...
  Surface cylinder;
  cylinder.make_cylinder(x0,y0,z0,x1,y1,z1,radius,edge_length); 
 ...
 *         f(x,y,z)=0  and the interior defined as f(x,y,z) < 0 (TODO) 
 * 
 ...
 * 
 * @tparam Inputiterator  TODO : remove -> overload is used anyway.
 ...
 * TODO Precondtion bounds volume.
...
/**
 * @brief Creates a circle in a plane. 
 *
 *
 *
 *
...
/**
 * TODO  combine with get_close_vertices or instead?  and move
 * @brief Finds close non-adjacent vertices and returns a map to move them apart.
 *
...
{  
   ...
   for (vertex_descriptor v_it : mesh.vertices())
   {
        ...
        closest = mesh.point((search.begin()+A)->first); 
        ...
        if (flag)
        {
           //direction = direction/CGAL::sqrt(CGAL::direction.squared_length());
           results[v_it] = adjustment*direction; //move away
        }
   }
   return  results;   
}
