...
class CGALSlice
{
    public :
       ...
       void keep_component(int next ) { Polyline_2 temp= constraints[next] ; constraints.clear();  constraints.push_back(temp);} ; // 0 largest
           
       void find_holes(int min_num_edges);

       int  num_constraints() { return constraints.size();}
       void create_mesh(double mesh_resolution);               
       void simplify(  double stop_crit); // simplify all polylines
