...
typedef SMS::Edge_profile<Smesh> Profile ;
...
struct My_visitor : SMS::Edge_collapse_visitor_base<Smesh> {
...
void edge_collapse(Smesh &surface_mesh) {
    // In this example, the simplification stops when the number of undirected edges
    // drops below 70% of the initial count
    SMS::Count_ratio_stop_predicate<Smesh> stop(0.01);
    ...
    int r = SMS::edge_collapse(
        surface_mesh,
        stop,
        CGAL::parameters::get_cost(SMS::Edge_length_cost <Smesh>())
            .get_placement(SMS::Midpoint_placement<Smesh>())
            .visitor(vis)
    );
