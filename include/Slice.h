// Copyright (C) 2018-2025 Lars Magnus Valnes
//
...
class Slice
{
   public :
      ...
      typedef CGAL::Polyline_simplification_2::Stop_above_cost_threshold Stop_above;
   ...
   */
   void create_mesh( double min_angle, double edge_size) 
   {
      ...
      double b = std::sin( min_angle*CGAL_PI/180.0)*std::sin( min_angle*CGAL_PI/180.0);
      Mesher mesher(cdt);    
 ...
 // DocString: connected_components
   ...
   */
   void add_constraint(std::vector<Point_2> polyline) 
   {  
   ...
   */
   ...
   /**
   * @brief Returns points that occurs in multiple constraints. 
   *
   * @returns a set of duplicate points.
   */     
   std::set<Point_2> duplicate_points()
   {
      ...
      for( auto constraint : this->constraints )  
      {
         for( auto pit : constraint) 
         {
            if( points.find(pit) != points.end() )
               duplicates.insert(pit);
            else 
               points.insert(pit);
         }
      } 
  ...
  /**
   * @brief reconstruct the constraints to a target edge length.      
   */
   void reconstruction(double target_edge_length)
   {
      CGAL::Polyline_simplification_2::simplify(cdt,Cost(),Stop_above(target_edge_length*target_edge_length));
   }
   ...
   * @brief reconstruct the constraints to a target edge length.      
   */
   void reconstruct_constraints(double target_edge_length, bool merge = true)
   {
      std::shared_ptr<Slice> temp(new Slice(this->plane));              
      temp->add_constraints(this->constraints);
      if(merge)
         temp->merge_close_bifurcation(target_edge_length);
      temp->set_constraints(); 
      temp->reconstruction(target_edge_length);
      auto simplified = temp->get_feature_points();
      //if(merge)
      //   merge_close_constraints(target_edge_length);
      this->constraints = simplified; 
   }

  //
  /**
   * @brief smooth_constraints 
   */
   void smooth_constraints( double beta)
   {
      std::vector<std::vector<Point_2>> updated;
      auto countmap = point_count(); 
      for( auto constraint : this->constraints )  
      {
         ...
         for( auto pit : constraint ) 
         {    
            if ( countmap[pit] > 1 and !segment.empty() )
            {
               segment.push_back(pit);
               segment = smooth_polyline<Point_2,Vector_2>(segment, beta);
               temp.insert(temp.end(), segment.begin(), segment.end());
               segment.clear();
            }
            else 
               segment.push_back(pit); 
  ...
  /**
   * @brief merges close bifurcations 
   */ 
   void merge_close_bifurcation( double min_threshold)
   {
      auto countmap = point_count(); 
      std::map<Point_2, Point_2> replace_map;
      std::vector<std::vector<Point_2>> updated;
      ...
      for( auto constraint : this->constraints )  
      {
         ...
         for( auto pit1 = constraint.begin(), pit2 = std::next(constraint.begin()); pit2 != constraint.end(); pit1++, pit2++ )
         {
            if( countmap[*pit1] > 1  and countmap[*pit2] > 1 and CGAL::squared_distance(*pit1, *pit2) < min_threshold*min_threshold) 
            {
               replace_map[*pit1] = *pit2;
               continue; 
            }  
            temp.push_back(*pit1);
         }       
         temp.push_back( constraint.back() );
         if ( temp.size()>1 ) 
            updated.push_back(temp);     
      }
      for( auto lit = updated.begin(); lit != updated.end(); lit++) 
      {
         for( auto pit = lit->begin(); pit != lit->end(); pit++)
         {
            if ( replace_map.find(*pit) != replace_map.end() )
               *pit = replace_map[*pit]; 
         }
      }
      this->constraints= updated; 
   ...
   */      
   void remove_constraints( std::vector<std::vector<Point_2>> polygons)
   {
      for( auto polygon : polygons) 
         remove_constraints(polygon);
   } 

  /**
   * @brief Removes any constraints that are inside a polygon.
   * TODO Discuss name remove_constraints_in or remove_constraints
   */    
   void remove_constraints(std::vector<Point_2> polygon)
   {
      ...
      for( auto pl = this->constraints.begin(); pl!= this->constraints.end();  )  
      {
         ...
         else 
         {
            ...
            if( count > 1 and size <= 2 + count )// Issues occure for points on boundary
   ...
   */      
   ...
   {
      assert_non_empty_mesh(); 
      typedef std::vector<std::size_t> Face_3D;
      typedef CDT::Vertex_handle Vertex_handle;
      std::vector<Point_3> points;
      ...
      void set_interface_tag(int subdomain_tag, int interface_tag)
      {
         ...
         for(Face_iterator fit=cdt.finite_faces_begin(); fit!=cdt.finite_faces_end(); ++fit) 
         {
            if( fit->info()!=subdomain_tag ) 
               continue; 
            for(int i=0; i<3; ++i) // FIXME Check consistency on ++i vs i++
            {
               Edge ei(fit,i); 
      ...
      * @param overwrite_tag, overwriting a specific integer marker, useful for holes in polygons  
      * @returns true if inside polygon domain, else false 
      */      
      ...
      */
      ...
      /**
      * @brief Returns constructed constrainted points
      * @param none  
      * @returns vector of points 
      */
      std::vector<std::vector<Point_2>> get_feature_points()
      {
         std::vector<std::vector<Point_2>> polylines;
         for( auto cid : this->cids ) 
         {    
            std::vector<Point_2> line; 
            for( auto pit = cdt.points_in_constraint_begin(cid); pit != cdt.points_in_constraint_end(cid); ++pit) 
               line.push_back(*pit);
