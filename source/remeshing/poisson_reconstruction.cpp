...
int main(int argc, char * argv[])
{
    ...
    // Poisson options
    FT sm_angle = 15.0; // Min triangle angle (degrees).
    FT sm_radius = 10; // Max triangle size w.r.t. point set average spacing.
    FT sm_distance = 0.21; // Approximation error w.r.t. point set average spacing.
    std::string solver_name = "eigen"; // Sparse linear solver name.
    double approximation_ratio = 0.15;       // 0.2
    double average_spacing_ratio = 1.5;      // 2
    ...
    if (extension == ".off" || extension == ".OFF") {
        ...
        BOOST_FOREACH(boost::graph_traits<Polyhedron>::vertex_descriptor v,
            ...
            Vector n = CGAL::Polygon_mesh_processing::compute_vertex_normal(v,
                    input_mesh);
    ...
    if (! points_have_normals) {
        std::cerr << "Error§ This reconstruction method requires oriented normals"
                  << std::endl;
        return EXIT_FAILURE;
    }

    CGAL::Timer reconstruction_timer;
    ...
    #ifdef CGAL_EIGEN3_ENABLED
    {
        CGAL::Eigen_solver_traits<
            Eigen::ConjugateGradient<CGAL::Eigen_sparse_symmetric_matrix<double>::EigenType> > solver;
    ...
    #else
    {
        std::cerr << "Error: CGAL is not built with Eigen3" << solver_name << "\n";
        return EXIT_FAILURE;
    }
    #endif

    // Prints status
    std::cerr << "Total implicit function (triangulation+refinement+solver): "
              << task_timer.time()
              << " seconds\n";
    task_timer.reset();
    ...
    if(inner_point_value >= 0.0) {
        std::cerr << "Error: unable to seed (" << inner_point_value
                  << " at inner_point)" << std::endl;
        return EXIT_FAILURE;
    }
    ...
    FT sm_sphere_radius = 3.0*radius;     //5.0
    ...
    std::cerr << "Reconstruction error:\n" << "  max = " << max_distance << " = "
              << max_distance/average_spacing << " * average spacing\n" << "  avg = " 
              << avg_distance << " = " << avg_distance/average_spacing
              << " * average spacing\n";
